		...
		auto timer_callback = [this]()->void {
            ...
            if (ids.size() > 0 && navstate == 20) // If at least one marker detected
                ...
                */
				...
				RCLCPP_INFO(this->get_logger(), "\033[97m Marker center deviation: time: %llu x: %f y: %f \033[0m",
                                irlock_msg.timestamp, irlock_msg.pos_x, irlock_msg.pos_y);
                };
...
int main(int argc, char* argv[])
{
	...
	std::cout << "31051810Starting offboard control node..." << std::endl;
